...
namespace json {
    ...
    bool value::equals(const value& rhs) const {
        ...
        std::clog << " this=" << this << ", rhs=" << &rhs << std::endl;
        ...
        std::clog << " this->type=" << type_names[this->m_type] << std::endl;
        */

/*
#define PRINT_VAL(x) \
            std::clog << " this val = " << m_u.x << ", rhs val = " << rhs.m_u.x << std::endl;
*/
...
#define PRINT_VAL(x)
        ...
        case DOUBLE:
            PRINT_VAL(d)
            // FIXME
            // What should we do? Compare the doubles, or only make sure 
            // that they don't differ more than the precision with which
            // we serialize them?
            return fabs(m_u.d - rhs.m_u.d) < 0.00001;
            //return m_u.d >= rhs.m_u.d && m_u.d <= rhs.m_u.d;
        case BOOLEAN:
            PRINT_VAL(b)
